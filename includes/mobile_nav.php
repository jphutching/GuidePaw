...
<style>
    ...
    .gp-bottom-nav { pointer-events: auto; max-width: 760px; margin: 0 auto; background: rgba(255,255,255,.97); backdrop-filter: blur(14px); border: 1px solid rgba(15,23,42,.12); border-bottom: 0; border-radius: 22px 22px 0 0; box-shadow: 0 -10px 28px rgba(15,23,42,.18); padding: .55rem .65rem calc(.55rem + env(safe-area-inset-bottom)); display: grid; grid-template-columns: repeat(5, 1fr); gap: .35rem; }
    .gp-bottom-nav a, .gp-bottom-nav button { border: 0; background: transparent; color: #1f2937; text-decoration: none; font-size: .72rem; line-height: 1.1; font-weight: 750; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: .18rem; border-radius: 14px; min-height: 48px; }
    ...
    .gp-menu-panel { display: none; position: fixed; left: 50%; bottom: 0; transform: translateX(-50%); z-index: 1051; width: min(760px, 100%); max-height: min(84vh, 760px); overflow: auto; background: #f8fafc; border-radius: 24px 24px 0 0; box-shadow: 0 -18px 42px rgba(15,23,42,.28); padding: 1rem 1rem calc(1rem + env(safe-area-inset-bottom)); }
    ...
    .gp-menu-description { color: #6b7280; font-size: .82rem; padding: 0 1rem .75rem; margin-top: -.15rem; }
    .gp-menu-link-grid { display: grid; grid-template-columns: 1fr 1fr; gap: .55rem; padding: 0 .85rem .9rem; }
    .gp-menu-link { display: flex; align-items: center; gap: .55rem; text-decoration: none; color: #1f2937; background: #f8fafc; border: 1px solid rgba(15,23,42,.08); border-radius: 14px; padding: .75rem; font-weight: 780; min-height: 48px; }
...
<aside class="gp-menu-panel" id="gpAppMenu" aria-label="GuidePaw menu">
    ...
    <?php if (gpNavIsAdmin()): ?><details class="gp-menu-section"><summary>🛠️ Admin</summary><div class="gp-menu-description">Beta access, found-dog reports, system health, feature flags, and backups.</div><div class="gp-menu-link-grid"><?php gpNavLink('admin.php', '🛠️', 'Admin Home'); ?><?php gpNavLink('admin_beta_requests.php', '📨', 'Beta Requests'); ?><?php gpNavLink('admin_found_dog_reports.php', '📍', 'Found Dog Reports'); ?><?php gpNavLink('admin_notification_test.php', '🔔', 'Notification Test'); ?><?php gpNavLink('admin_feedback.php', '💬', 'Feedback Reports'); ?><?php gpNavLink('admin_users.php', '👥', 'User Management'); ?><?php gpNavLink('admin_feature_roadmap.php', '🗺️', 'Feature Roadmap'); ?><?php gpNavLink('api_tokens.php', '🔐', 'API Tokens'); ?><?php gpNavLink('db_status.php', '🩺', 'System Health'); ?><?php gpNavLink('backup.php', '💾', 'Backup Tools', 'backup_tools_enabled'); ?></div></details><?php endif; ?>
</aside>
