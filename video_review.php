...
if (!featureEnabled($pdo, 'video_reviews_enabled')) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($allowedLog > 0 && $canEdit && $tableReady) {
        gpSaveMediaReview($pdo, $userId, $allowedLog, $_POST);
        writeAuditLog($pdo, 'video_review_saved', 'daily_logs', $allowedLog, 'Saved video checkpoint review.');
        header('Location: video_review.php?msg=saved#log-' . $allowedLog);
        exit;
    }

    $error = $canEdit ? 'You can only review videos that belong to this dog.' : 'You have read-only access for this dog.';
}

$entries = gpVideoReviewLogEntries($pdo, $userId, $dogId, 12);
...
<header class="hero">
    <div class="review-shell px-0">
        <div class="d-flex justify-content-between align-items-start gap-3 flex-wrap">
            <div>
                <div class="small opacity-75">GuidePaw checkpoint video review</div>
                <h1 class="mb-2">Video Review</h1>
                <p class="mb-0 opacity-75">Review short checkpoint clips for frame quality, audio clarity, and whether the dog is ready to advance.</p>
            </div>
...
<main class="review-shell">
    ...
    <?php if (!$tableReady): ?><div class="alert alert-warning">Video review storage has not been deployed in this database yet.</div><?php endif; ?>
    <?php if (!$entries): ?><div class="alert alert-info">No checkpoint videos are available for this dog yet.</div><?php else: ?>
        <div class="review-grid">
            <?php foreach ($entries as $entry): ?>
                <details class="review-card fold-card" id="log-<?= (int) $entry['id'] ?>">
                    <summary>
                        <div class="min-w-0">
                    ...
                    <div class="p-3 p-md-4">
                        ...
                        <div class="mt-3">
                            <video class="w-100 inline-video rounded border" controls preload="metadata">
                        ...
                        <?php if (!empty($entry['reviewed_at'])): ?>
                            <div class="mt-3">
                                <span class="pill">Frame <?= (int) $entry['rating_camera_stability'] ?>/5</span>
                                <span class="pill">Audio <?= (int) $entry['rating_audio_clarity'] ?>/5</span>
                                <span class="pill">Ready <?= (int) $entry['rating_training_value'] ?>/5</span>
                        ...
                        <?php if ($canEdit && $tableReady): ?>
                            <form method="post" class="mt-4">
                                ...
                                <div class="review-grid-form">
                                    <div>
                                        <label class="form-label fw-bold">Frame quality</label>
                                    ...
                                    <div>
                                        <label class="form-label fw-bold">Checkpoint readiness</label>
                                ...
                                <div class="mt-3">
                                    ...
                                    <textarea class="form-control" name="review_notes" rows="3" placeholder="What does the clip show?"><?= e($entry['review_notes'] ?? '') ?></textarea>
                                </div>
                                <div class="mt-3">
                                ...
                                <div class="d-grid mt-3">
                                    <button class="btn btn-primary" type="submit">Save video review</button>
                                </div>
                            </form>
                        <?php elseif ($canEdit): ?>
