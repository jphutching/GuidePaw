    ...
    <td>
        <div class="d-flex flex-column gap-2">
            ...
            <?php elseif ($protected): ?>
                <span class="text-danger small fw-bold">Built-in admin cannot be downgraded or deactivated.</span>
            <?php else: ?>
                <form method="post" class="border rounded p-2 bg-light">
                    <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
                    <input type="hidden" name="user_id" value="<?= (int) $u['id'] ?>">
                    <input type="hidden" name="action" value="set_role">
                    <label class="form-label small mb-1">Change role. Type exactly: <code><?= e($label) ?></code></label>
                    <div class="row g-1">
                        <div class="col-md-4">
                            <select class="form-select form-select-sm" name="user_role">
                                <?php foreach (auRoleOptions() as $value => $labelText): ?>
                                    <option value="<?= e($value) ?>" <?= $role === $value ? 'selected' : '' ?>><?= e($labelText) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
