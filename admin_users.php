...
<div class="alert alert-info"><strong>Roles:</strong> Admin can access beta/admin checks and full admin tools. Moderator can support/review permitted tools. User can access regular site features. The built-in <code>admin</code> account is protected. Hard purge stays disabled during beta; export data first, then deactivate when needed.</div>
...
<?php foreach ($users as $u): ?><?php $label = auUserLabel($u); $role = gpUserRole($u); $protected = auIsBuiltInAdmin($u); ?><tr><td><?= (int)$u['id'] ?></td><td><strong><?= e($u['username'] ?? '') ?></strong><?= $protected ? ' <span class="badge text-bg-danger">Protected</span>' : '' ?><br><span class="text-muted"><?= e($u['email'] ?? '') ?></span></td><td><?= e($u['full_name'] ?? '') ?><br><span class="text-muted"><?= e($u['phone'] ?? '') ?></span></td><td><?= e($u['dog_name'] ?? '') ?></td><td><span class="badge <?= $u['account_status'] === 'active' ? 'bg-success' : 'bg-secondary' ?>"><?= e($u['account_status']) ?></span><?php if ($u['deactivated_at']): ?><br><small><?= e($u['deactivated_at']) ?></small><?php endif; ?></td><td><?= auRoleBadge($role) ?></td><td><?= e($u['created_at'] ?? '') ?></td><td><div class="d-flex flex-column gap-2"><a class="btn btn-sm btn-outline-primary" href="admin_users.php?action=export&user_id=<?= (int)$u['id'] ?>">Download user data JSON</a><?php if ((int)$u['id'] === (int)$_SESSION['user_id']): ?><span class="text-muted small">Current admin account cannot be changed here.</span><?php elseif ($protected): ?><span class="text-danger small fw-bold">Built-in admin cannot be downgraded or deactivated.</span><?php else: ?>
