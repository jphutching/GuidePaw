...
{
    $stmt = $pdo->prepare("SELECT dh.*, u.username, u.email, u.display_name FROM dog_handlers dh JOIN users u ON u.id = dh.user_id WHERE dh.dog_id = ? ORDER BY CASE dh.status WHEN 'accepted' THEN 1 WHEN 'pending' THEN 2 WHEN 'declined' THEN 3 WHEN 'revoked' THEN 4 ELSE 5 END, u.username ASC");
    $stmt->execute([$dogId]);
