...
{
    $stmt = $pdo->prepare("SELECT d.*, u.username AS owner_username FROM dogs d JOIN users u ON u.id = d.owner_user_id LEFT JOIN dog_handlers dh ON dh.dog_id = d.id AND dh.user_id = ? AND dh.status = 'accepted' WHERE d.id = ? AND (d.owner_user_id = ? OR dh.id IS NOT NULL) LIMIT 1");
...
{
    $stmt = $pdo->prepare("SELECT dh.*, u.username, u.email, u.display_name FROM dog_handlers dh JOIN users u ON u.id = dh.user_id WHERE dh.dog_id = ? ORDER BY CASE dh.status WHEN 'accepted' THEN 1 WHEN 'pending' THEN 2 WHEN 'revoked' THEN 3 ELSE 4 END, u.username ASC");
    $stmt->execute([$dogId]);
    ...
    if (!$errors && $action === 'grant_access') {
        if (!gpDogAccessOwnerOnly($dog, $userId)) { $errors[] = 'Only the dog owner can grant handler access.'; }
            ...
            elseif ((int) $target['id'] === $userId) { $errors[] = 'You already own this dog profile.'; }
                ...
                gpDogAccessNotifySharedGranted($dog, $owner, $target, $role, $permission, $endsAt ?: null);
