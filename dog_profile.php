...
function gpEnsureDogPublicProfileColumns(PDO $pdo): void
{
    $columns = [
        ...
        'handler_photo_url' => 'TEXT',
        'handler_name' => 'TEXT',
        'handler_phone' => 'TEXT',
        'handler_email' => 'TEXT',
        'backup_contact_name' => 'TEXT',
        'backup_contact_phone' => 'TEXT',
        'found_dog_instructions' => 'TEXT',
        'public_notes' => 'TEXT',
        'service_tasks' => 'TEXT',
...
function gpSavePublicProfileImage(string $field, ?string $existingPath, array &$errors): ?string
{
    if (empty($_FILES[$field]) || ($_FILES[$field]['error'] ?? UPLOAD_ERR_NO_FILE) === UPLOAD_ERR_NO_FILE) {
        return $existingPath;
    }
    if (($_FILES[$field]['error'] ?? UPLOAD_ERR_OK) !== UPLOAD_ERR_OK) {
        $errors[] = 'Image upload failed for ' . $field . '.';
        return $existingPath;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $canEdit) {
    ...
    $dogPhoto = gpSavePublicProfileImage('profile_photo', $dog['profile_photo_url'] ?? null, $errors);
    $handlerPhoto = gpSavePublicProfileImage('handler_photo', $dog['handler_photo_url'] ?? null, $errors);
...
.breed-search-option:last-child{border-bottom:0;}.breed-search-option:hover,.breed-search-option:focus{background:#f8fafc;outline:0;}.breed-search-name{display:block;font-weight:600;}.breed-search-meta{display:block;color:#6c757d;font-size:.82rem;margin-top:2px;}.breed-search-empty{padding:11px 12px;color:#6c757d;}
.profile-photo-preview{width:86px;height:86px;border-radius:18px;object-fit:cover;background:#eef2f7;border:1px solid #dbe3ef;}
.qr-card{text-align:center;}.qr-card img{max-width:260px;width:100%;height:auto;border:1px solid #e5e7eb;padding:.5rem;background:#fff;border-radius:14px;}
</style>
</head>
<body class="pb-5">
...
<div class="container py-4" style="max-width: 820px;">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div><h2 class="mb-0">🪪 <?= e($dog['name']) ?></h2><small class="text-muted">Owner: <?= e($dog['owner_username']) ?></small></div>
        <div class="d-flex gap-2"><a href="dogs.php" class="btn btn-outline-secondary btn-sm">Dogs</a><a href="index.php?set_dog=<?= (int) $dog['id'] ?>" class="btn btn-outline-primary btn-sm">Make Active</a></div>
    </div>

    <?php if (!empty($_GET['status'])): ?><div class="alert alert-success">Dog profile saved.</div><?php endif; ?>
    <?php if ($errors): ?><div class="alert alert-danger"><ul class="mb-0"><?php foreach ($errors as $error): ?><li><?= e($error) ?></li><?php endforeach; ?></ul></div><?php endif; ?>

    <div class="card shadow-sm mb-3 qr-card"><div class="card-body">
        <h3 class="h5">Public QR Profile</h3>
        <p class="text-muted small mb-3">This unique QR code opens a public, no-login contact page for this dog only.</p>
        <img src="<?= e($qrUrl) ?>" alt="Public QR profile for <?= e($dog['name']) ?>">
        <div class="d-grid gap-2 mt-3">
            <a class="btn btn-outline-primary" href="<?= e($publicUrl) ?>" target="_blank" rel="noopener">Preview Public Profile</a>
            <button type="button" class="btn btn-outline-secondary" id="copyPublicUrl">Copy Public Link</button>
        </div>
        <div class="small text-muted mt-2" id="copyStatus"></div>
    </div></div>
    ...
    <div class="card shadow-sm"><div class="card-body">
        ...
        <form method="post" enctype="multipart/form-data" class="row g-3">
            ...
            <div class="col-md-6"><label class="form-label">Birthday</label><input type="date" name="date_of_birth" class="form-control" value="<?= e((string) $dog['date_of_birth']) ?>" <?= $canEdit ? '' : 'disabled' ?>></div>
            <div class="col-md-6"><label class="form-label">Approx Age (years)</label><input type="number" step="0.1" name="approx_age_years" class="form-control" value="<?= e((string) $dog['approx_age_years']) ?>" <?= $canEdit ? '' : 'disabled' ?>></div>
            ...
            <div class="col-12"><label class="form-label">Private Notes</label><textarea name="notes" class="form-control" rows="3" <?= $canEdit ? '' : 'disabled' ?>><?= e($dog['notes']) ?></textarea><div class="form-text">Private app notes. These do not show on the public QR profile.</div></div>
            ...
            <div class="col-md-6"><label class="form-label">Dog Profile Picture</label><div class="d-flex gap-3 align-items-center"><?php if (!empty($dog['profile_photo_url'])): ?><img src="<?= e($dog['profile_photo_url']) ?>" class="profile-photo-preview" alt="Dog photo"><?php else: ?><div class="profile-photo-preview d-flex align-items-center justify-content-center">🐕</div><?php endif; ?><input type="file" name="profile_photo" class="form-control" accept="image/jpeg,image/png,image/webp" <?= $canEdit ? '' : 'disabled' ?>></div></div>
            <div class="col-md-6"><label class="form-label">Handler Picture</label><div class="d-flex gap-3 align-items-center"><?php if (!empty($dog['handler_photo_url'])): ?><img src="<?= e($dog['handler_photo_url']) ?>" class="profile-photo-preview" alt="Handler photo"><?php else: ?><div class="profile-photo-preview d-flex align-items-center justify-content-center">👤</div><?php endif; ?><input type="file" name="handler_photo" class="form-control" accept="image/jpeg,image/png,image/webp" <?= $canEdit ? '' : 'disabled' ?>></div></div>
            <div class="col-md-4"><label class="form-label">Handler Name</label><input type="text" name="handler_name" class="form-control" value="<?= e($dog['handler_name'] ?? ($dog['owner_username'] ?? '')) ?>" <?= $canEdit ? '' : 'disabled' ?>></div>
            <div class="col-md-4"><label class="form-label">Handler Phone</label><input type="text" name="handler_phone" class="form-control" value="<?= e($dog['handler_phone'] ?? '') ?>" <?= $canEdit ? '' : 'disabled' ?>></div>
            <div class="col-md-4"><label class="form-label">Handler Email</label><input type="email" name="handler_email" class="form-control" value="<?= e($dog['handler_email'] ?? ($dog['owner_email'] ?? '')) ?>" <?= $canEdit ? '' : 'disabled' ?>></div>
            <div class="col-md-6"><label class="form-label">Backup Contact Name</label><input type="text" name="backup_contact_name" class="form-control" value="<?= e($dog['backup_contact_name'] ?? '') ?>" <?= $canEdit ? '' : 'disabled' ?>></div>
            <div class="col-md-6"><label class="form-label">Backup Contact Phone</label><input type="text" name="backup_contact_phone" class="form-control" value="<?= e($dog['backup_contact_phone'] ?? '') ?>" <?= $canEdit ? '' : 'disabled' ?>></div>
            <div class="col-12"><label class="form-label">If Found / Emergency Instructions</label><textarea name="found_dog_instructions" class="form-control" rows="3" placeholder="Example: Please call handler first. If no answer, call backup contact or primary vet." <?= $canEdit ? '' : 'disabled' ?>><?= e($dog['found_dog_instructions'] ?? '') ?></textarea></div>
            <div class="col-12"><label class="form-label">Public Service Task Notes</label><textarea name="service_tasks" class="form-control" rows="3" placeholder="Keep this general. Do not disclose diagnosis details." <?= $canEdit ? '' : 'disabled' ?>><?= e($dog['service_tasks'] ?? '') ?></textarea></div>
            <div class="col-12"><label class="form-label">Critical Medical / Allergy Note</label><textarea name="critical_allergies" class="form-control" rows="3" placeholder="Only include urgent public safety information." <?= $canEdit ? '' : 'disabled' ?>><?= e($dog['critical_allergies'] ?? '') ?></textarea></div>
            <div class="col-12"><label class="form-label">Public Handler Notes</label><textarea name="public_notes" class="form-control" rows="3" placeholder="Optional public note for someone who scans the QR code." <?= $canEdit ? '' : 'disabled' ?>><?= e($dog['public_notes'] ?? '') ?></textarea></div>
