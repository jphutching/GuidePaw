...
{
    $columns = [
        'display_name' => 'TEXT', 'home_address' => 'TEXT', 'phone' => 'TEXT', 'public_email' => 'TEXT', 'home_state' => 'TEXT', 'profile_photo_url' => 'TEXT',
...
$handlerStmt = $pdo->prepare('SELECT username, email, display_name, home_address, phone, public_email, home_state, profile_photo_url, backup_contact_name, backup_contact_phone, public_notes FROM users WHERE id = ? LIMIT 1');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $canEdit) {
    ...
    if ($action === 'use_handler_defaults') {
        ...
        $defaultAddress = cleanText((string) ($handlerProfile['home_address'] ?? ''), 255);
