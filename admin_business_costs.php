...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'import_porkbun_csv') {
            ...
            $result = ['ok' => false, 'error' => 'Paste the Porkbun order history CSV first.'];
            if ($csv !== '') {
            ...
            writeAuditLog($pdo, 'vendor_cost_imported', 'vendor_cost_imports', null, 'Imported Porkbun ' . ((string) ($result['source_label'] ?? 'CSV')) . ' for ' . ($result['period_label'] ?? 'the selected period') . '.');
