...
echo 'GuidePaw local QA crawler targeting ' . $baseUrl . ($insecureLocalSsl ? ' with local SSL verification disabled' : '') . PHP_EOL;
    ...
    if ($adminLoggedIn) {
        ...
        $breedComparisonAussiePage = gpQaRequest($baseUrl, 'breed_compare_aussie_mini_american_shepherd.php', 'GET', [], '', $insecureLocalSsl, '', false);
        $breedComparisonCorgiPage = gpQaRequest($baseUrl, 'breed_compare_pembroke_cardigan_corgi.php', 'GET', [], '', $insecureLocalSsl, '', false);
        $breedComparisonFrenchiePage = gpQaRequest($baseUrl, 'breed_compare_french_bulldog_boston_terrier.php', 'GET', [], '', $insecureLocalSsl, '', false);
        ...
        $apiWearableOverview = ['status' => 0, 'body' => '', 'headers' => '', 'url' => '', 'error' => 'skipped'];
        $apiBilling = ['status' => 0, 'body' => '', 'headers' => '', 'url' => '', 'error' => 'skipped'];
        $apiMeSeen = false;
        $apiDogsSeen = false;
        $apiLogsSeen = false;
        $apiWearablesSeen = false;
        $apiWearableOverviewSeen = false;
        ...
        $breedComparisonCorgiSeen = gpQaPageLooksOk($breedComparisonCorgiPage) && str_contains(strtolower($breedComparisonCorgiPage['body']), 'pembroke welsh corgi vs cardigan welsh corgi');
        $breedComparisonFrenchieSeen = gpQaPageLooksOk($breedComparisonFrenchiePage) && str_contains(strtolower($breedComparisonFrenchiePage['body']), 'french bulldog vs boston terrier');
        gpQaResult($results, 'breed_compare_labrador_golden_page_loads', $breedComparisonLabSeen, 'HTTP ' . $breedComparisonLabPage['status'] . ($breedComparisonLabSeen ? ' retriever comparison found' : ' retriever comparison missing'));
        gpQaResult($results, 'breed_compare_standard_toy_poodle_page_loads', $breedComparisonPoodleSeen, 'HTTP ' . $breedComparisonPoodlePage['status'] . ($breedComparisonPoodleSeen ? ' poodle comparison found' : ' poodle comparison missing'));
        gpQaResult($results, 'breed_compare_aussie_mini_american_shepherd_page_loads', $breedComparisonAussieSeen, 'HTTP ' . $breedComparisonAussiePage['status'] . ($breedComparisonAussieSeen ? ' herding comparison found' : ' herding comparison missing'));
        gpQaResult($results, 'breed_compare_pembroke_cardigan_corgi_page_loads', $breedComparisonCorgiSeen, 'HTTP ' . $breedComparisonCorgiPage['status'] . ($breedComparisonCorgiSeen ? ' corgi comparison found' : ' corgi comparison missing'));
        gpQaResult($results, 'breed_compare_french_bulldog_boston_terrier_page_loads', $breedComparisonFrenchieSeen, 'HTTP ' . $breedComparisonFrenchiePage['status'] . ($breedComparisonFrenchieSeen ? ' compact companion comparison found' : ' compact companion comparison missing'));
    ...
    $breedQuestionnaireToyManchesterQueryPage = gpQaRequest($baseUrl, 'breed_questionnaire.php', 'POST', [
        'goal' => 'companion',
        'breed_query' => 'Toy Manchester Terrier',
        'size' => 'flexible',
        'energy' => 'moderate',
        'grooming' => 'moderate',
        'public' => 'some',
    ...
    $breedQuestionnaireToyManchesterQueryBody = strtolower($breedQuestionnaireToyManchesterQueryPage['body']);
    $breedQuestionnaireToyManchesterQuerySeen = gpQaPageLooksOk($breedQuestionnaireToyManchesterQueryPage)
        && str_contains($breedQuestionnaireToyManchesterQueryBody, 'manchester terrier (toy)');
    $breedQuestionnaireToyManchesterQueryAlignmentSeen = false;
    if (gpQaPageLooksOk($breedQuestionnaireToyManchesterQueryPage) && preg_match('/<h2 class="h5 mb-3">Top breed ideas<\/h2>.*?<div class="fw-bold">([^<]+)<\/div>/is', $breedQuestionnaireToyManchesterQueryPage['body'], $breedQuestionnaireToyManchesterQueryMatch)) {
        $firstBreed = trim((string) ($breedQuestionnaireToyManchesterQueryMatch[1] ?? ''));
        $breedQuestionnaireToyManchesterQueryAlignmentSeen = strcasecmp($firstBreed, 'Manchester Terrier (Toy)') === 0;
    }
    $breedQuestionnaireMinPinQueryPage = gpQaRequest($baseUrl, 'breed_questionnaire.php', 'POST', [
        'goal' => 'companion',
        'breed_query' => 'Min Pin',
        'size' => 'flexible',
        'energy' => 'moderate',
    ...
    $breedQuestionnaireMinPinQueryBody = strtolower($breedQuestionnaireMinPinQueryPage['body']);
    $breedQuestionnaireMinPinQuerySeen = gpQaPageLooksOk($breedQuestionnaireMinPinQueryPage)
        && str_contains($breedQuestionnaireMinPinQueryBody, 'miniature pinscher');
    $breedQuestionnaireMinPinQueryAlignmentSeen = false;
    if (gpQaPageLooksOk($breedQuestionnaireMinPinQueryPage) && preg_match('/<h2 class="h5 mb-3">Top breed ideas<\/h2>.*?<div class="fw-bold">([^<]+)<\/div>/is', $breedQuestionnaireMinPinQueryPage['body'], $breedQuestionnaireMinPinQueryMatch)) {
        $firstBreed = trim((string) ($breedQuestionnaireMinPinQueryMatch[1] ?? ''));
        $breedQuestionnaireMinPinQueryAlignmentSeen = strcasecmp($firstBreed, 'Miniature Pinscher') === 0;
    }
    $breedQuestionnaireRussianToyQueryPage = gpQaRequest($baseUrl, 'breed_questionnaire.php', 'POST', [
        'goal' => 'companion',
        'breed_query' => 'Russian Toy Terrier',
        'size' => 'flexible',
        'energy' => 'moderate',
        'grooming' => 'moderate',
        'public' => 'some',
    ...
    $breedQuestionnaireRussianToyQueryBody = strtolower($breedQuestionnaireRussianToyQueryPage['body']);
    $breedQuestionnaireRussianToyQuerySeen = gpQaPageLooksOk($breedQuestionnaireRussianToyQueryPage)
        && str_contains($breedQuestionnaireRussianToyQueryBody, 'russian toy');
    $breedQuestionnaireRussianToyQueryAlignmentSeen = false;
    if (gpQaPageLooksOk($breedQuestionnaireRussianToyQueryPage) && preg_match('/<h2 class="h5 mb-3">Top breed ideas<\/h2>.*?<div class="fw-bold">([^<]+)<\/div>/is', $breedQuestionnaireRussianToyQueryPage['body'], $breedQuestionnaireRussianToyQueryMatch)) {
        $firstBreed = trim((string) ($breedQuestionnaireRussianToyQueryMatch[1] ?? ''));
        $breedQuestionnaireRussianToyQueryAlignmentSeen = strcasecmp($firstBreed, 'Russian Toy') === 0;
    }
    $breedQuestionnaireSilkyQueryPage = gpQaRequest($baseUrl, 'breed_questionnaire.php', 'POST', [
        'goal' => 'companion',
        'breed_query' => 'Silky',
        'size' => 'flexible',
        'energy' => 'moderate',
    ...
    $breedQuestionnaireSilkyQueryBody = strtolower($breedQuestionnaireSilkyQueryPage['body']);
    $breedQuestionnaireSilkyQuerySeen = gpQaPageLooksOk($breedQuestionnaireSilkyQueryPage)
        && str_contains($breedQuestionnaireSilkyQueryBody, 'silky terrier');
    $breedQuestionnaireSilkyQueryAlignmentSeen = false;
    if (gpQaPageLooksOk($breedQuestionnaireSilkyQueryPage) && preg_match('/<h2 class="h5 mb-3">Top breed ideas<\/h2>.*?<div class="fw-bold">([^<]+)<\/div>/is', $breedQuestionnaireSilkyQueryPage['body'], $breedQuestionnaireSilkyQueryMatch)) {
        $firstBreed = trim((string) ($breedQuestionnaireSilkyQueryMatch[1] ?? ''));
        $breedQuestionnaireSilkyQueryAlignmentSeen = strcasecmp($firstBreed, 'Silky Terrier') === 0;
    }
    $breedQuestionnaireAmericanCockerQueryPage = gpQaRequest($baseUrl, 'breed_questionnaire.php', 'POST', [
        'goal' => 'companion',
        'breed_query' => 'American Cocker',
        'size' => 'flexible',
        'energy' => 'moderate',
        'grooming' => 'moderate',
        'public' => 'some',
    ...
    $breedQuestionnaireAmericanCockerQueryBody = strtolower($breedQuestionnaireAmericanCockerQueryPage['body']);
    $breedQuestionnaireAmericanCockerQuerySeen = gpQaPageLooksOk($breedQuestionnaireAmericanCockerQueryPage)
        && str_contains($breedQuestionnaireAmericanCockerQueryBody, 'american cocker spaniel');
    $breedQuestionnaireAmericanCockerQueryAlignmentSeen = false;
    if (gpQaPageLooksOk($breedQuestionnaireAmericanCockerQueryPage) && preg_match('/<h2 class="h5 mb-3">Top breed ideas<\/h2>.*?<div class="fw-bold">([^<]+)<\/div>/is', $breedQuestionnaireAmericanCockerQueryPage['body'], $breedQuestionnaireAmericanCockerQueryMatch)) {
        $firstBreed = trim((string) ($breedQuestionnaireAmericanCockerQueryMatch[1] ?? ''));
        $breedQuestionnaireAmericanCockerQueryAlignmentSeen = strcasecmp($firstBreed, 'American Cocker Spaniel') === 0;
    }
    $breedQuestionnaireDoxieQueryPage = gpQaRequest($baseUrl, 'breed_questionnaire.php', 'POST', [
        'goal' => 'companion',
        'breed_query' => 'Doxie',
        'size' => 'flexible',
        'energy' => 'moderate',
    ...
    $breedQuestionnaireDoxieQueryBody = strtolower($breedQuestionnaireDoxieQueryPage['body']);
    $breedQuestionnaireDoxieQuerySeen = gpQaPageLooksOk($breedQuestionnaireDoxieQueryPage)
        && str_contains($breedQuestionnaireDoxieQueryBody, 'dachshund');
    $breedQuestionnaireDoxieQueryAlignmentSeen = false;
    if (gpQaPageLooksOk($breedQuestionnaireDoxieQueryPage) && preg_match('/<h2 class="h5 mb-3">Top breed ideas<\/h2>.*?<div class="fw-bold">([^<]+)<\/div>/is', $breedQuestionnaireDoxieQueryPage['body'], $breedQuestionnaireDoxieQueryMatch)) {
        $firstBreed = trim((string) ($breedQuestionnaireDoxieQueryMatch[1] ?? ''));
        $breedQuestionnaireDoxieQueryAlignmentSeen = strcasecmp($firstBreed, 'Dachshund') === 0;
    }
    $breedQuestionnairePembrokeCorgiQueryPage = gpQaRequest($baseUrl, 'breed_questionnaire.php', 'POST', [
        'goal' => 'companion',
        'breed_query' => 'Pembroke Corgi',
        'size' => 'flexible',
        'energy' => 'moderate',
        'grooming' => 'moderate',
        'public' => 'some',
    ...
    $breedQuestionnairePembrokeCorgiQueryBody = strtolower($breedQuestionnairePembrokeCorgiQueryPage['body']);
    $breedQuestionnairePembrokeCorgiQuerySeen = gpQaPageLooksOk($breedQuestionnairePembrokeCorgiQueryPage)
        && str_contains($breedQuestionnairePembrokeCorgiQueryBody, 'pembroke welsh corgi');
    $breedQuestionnairePembrokeCorgiQueryAlignmentSeen = false;
    if (gpQaPageLooksOk($breedQuestionnairePembrokeCorgiQueryPage) && preg_match('/<h2 class="h5 mb-3">Top breed ideas<\/h2>.*?<div class="fw-bold">([^<]+)<\/div>/is', $breedQuestionnairePembrokeCorgiQueryPage['body'], $breedQuestionnairePembrokeCorgiQueryMatch)) {
        $firstBreed = trim((string) ($breedQuestionnairePembrokeCorgiQueryMatch[1] ?? ''));
        $breedQuestionnairePembrokeCorgiQueryAlignmentSeen = strcasecmp($firstBreed, 'Pembroke Welsh Corgi') === 0;
    }
    $breedQuestionnaireBostonQueryPage = gpQaRequest($baseUrl, 'breed_questionnaire.php', 'POST', [
        'goal' => 'companion',
        'breed_query' => 'Boston',
        'size' => 'flexible',
        'energy' => 'moderate',
    ...
    $breedQuestionnaireBostonQueryBody = strtolower($breedQuestionnaireBostonQueryPage['body']);
    $breedQuestionnaireBostonQuerySeen = gpQaPageLooksOk($breedQuestionnaireBostonQueryPage)
        && str_contains($breedQuestionnaireBostonQueryBody, 'boston terrier');
    $breedQuestionnaireBostonQueryAlignmentSeen = false;
    if (gpQaPageLooksOk($breedQuestionnaireBostonQueryPage) && preg_match('/<h2 class="h5 mb-3">Top breed ideas<\/h2>.*?<div class="fw-bold">([^<]+)<\/div>/is', $breedQuestionnaireBostonQueryPage['body'], $breedQuestionnaireBostonQueryMatch)) {
    ...
    $breedQuestionnaireBlueHeelerQueryPage = gpQaRequest($baseUrl, 'breed_questionnaire.php', 'POST', [
        'goal' => 'companion',
        'breed_query' => 'Blue Heeler',
        'size' => 'flexible',
        'energy' => 'moderate',
        'grooming' => 'moderate',
        'public' => 'some',
    ...
    $breedQuestionnaireBlueHeelerQueryBody = strtolower($breedQuestionnaireBlueHeelerQueryPage['body']);
    $breedQuestionnaireBlueHeelerQuerySeen = gpQaPageLooksOk($breedQuestionnaireBlueHeelerQueryPage)
        && str_contains($breedQuestionnaireBlueHeelerQueryBody, 'australian cattle dog');
    $breedQuestionnaireBlueHeelerQueryAlignmentSeen = false;
    if (gpQaPageLooksOk($breedQuestionnaireBlueHeelerQueryPage) && preg_match('/<h2 class="h5 mb-3">Top breed ideas<\/h2>.*?<div class="fw-bold">([^<]+)<\/div>/is', $breedQuestionnaireBlueHeelerQueryPage['body'], $breedQuestionnaireBlueHeelerQueryMatch)) {
        $firstBreed = trim((string) ($breedQuestionnaireBlueHeelerQueryMatch[1] ?? ''));
        $breedQuestionnaireBlueHeelerQueryAlignmentSeen = strcasecmp($firstBreed, 'Australian Cattle Dog') === 0;
    }
    $breedQuestionnaireFrenchieQueryPage = gpQaRequest($baseUrl, 'breed_questionnaire.php', 'POST', [
        'goal' => 'companion',
        'breed_query' => 'Frenchie',
        'size' => 'flexible',
        'energy' => 'moderate',
        'grooming' => 'moderate',
        'public' => 'some',
    ...
    $breedQuestionnaireFrenchieQueryBody = strtolower($breedQuestionnaireFrenchieQueryPage['body']);
    $breedQuestionnaireFrenchieQuerySeen = gpQaPageLooksOk($breedQuestionnaireFrenchieQueryPage)
        && str_contains($breedQuestionnaireFrenchieQueryBody, 'french bulldog');
    $breedQuestionnaireFrenchieQueryAlignmentSeen = false;
    if (gpQaPageLooksOk($breedQuestionnaireFrenchieQueryPage) && preg_match('/<h2 class="h5 mb-3">Top breed ideas<\/h2>.*?<div class="fw-bold">([^<]+)<\/div>/is', $breedQuestionnaireFrenchieQueryPage['body'], $breedQuestionnaireFrenchieQueryMatch)) {
    ...
    $breedQuestionnaireLabQueryPage = gpQaRequest($baseUrl, 'breed_questionnaire.php', 'POST', [
        'goal' => 'companion',
        'breed_query' => 'Lab',
        'size' => 'flexible',
        'energy' => 'moderate',
    ...
    $breedQuestionnaireLabQueryBody = strtolower($breedQuestionnaireLabQueryPage['body']);
    $breedQuestionnaireLabQuerySeen = gpQaPageLooksOk($breedQuestionnaireLabQueryPage)
        && str_contains($breedQuestionnaireLabQueryBody, 'labrador retriever');
    $breedQuestionnaireLabQueryAlignmentSeen = false;
    if (gpQaPageLooksOk($breedQuestionnaireLabQueryPage) && preg_match('/<h2 class="h5 mb-3">Top breed ideas<\/h2>.*?<div class="fw-bold">([^<]+)<\/div>/is', $breedQuestionnaireLabQueryPage['body'], $breedQuestionnaireLabQueryMatch)) {
        $firstBreed = trim((string) ($breedQuestionnaireLabQueryMatch[1] ?? ''));
        $breedQuestionnaireLabQueryAlignmentSeen = strcasecmp($firstBreed, 'Labrador Retriever') === 0;
    }
    $breedQuestionnaireGspQueryPage = gpQaRequest($baseUrl, 'breed_questionnaire.php', 'POST', [
        'goal' => 'companion',
        'breed_query' => 'GSP',
        'size' => 'flexible',
        'energy' => 'moderate',
    ...
    $breedQuestionnaireGspQueryBody = strtolower($breedQuestionnaireGspQueryPage['body']);
    $breedQuestionnaireGspQuerySeen = gpQaPageLooksOk($breedQuestionnaireGspQueryPage)
        && str_contains($breedQuestionnaireGspQueryBody, 'german shorthaired pointer');
    $breedQuestionnaireGspQueryAlignmentSeen = false;
    if (gpQaPageLooksOk($breedQuestionnaireGspQueryPage) && preg_match('/<h2 class="h5 mb-3">Top breed ideas<\/h2>.*?<div class="fw-bold">([^<]+)<\/div>/is', $breedQuestionnaireGspQueryPage['body'], $breedQuestionnaireGspQueryMatch)) {
        $firstBreed = trim((string) ($breedQuestionnaireGspQueryMatch[1] ?? ''));
        $breedQuestionnaireGspQueryAlignmentSeen = strcasecmp($firstBreed, 'German Shorthaired Pointer') === 0;
    }
    $breedQuestionnaireMalQueryPage = gpQaRequest($baseUrl, 'breed_questionnaire.php', 'POST', [
        'goal' => 'companion',
        'breed_query' => 'Mal',
        'size' => 'flexible',
        'energy' => 'moderate',
    ...
    $breedQuestionnaireMalQueryBody = strtolower($breedQuestionnaireMalQueryPage['body']);
    $breedQuestionnaireMalQuerySeen = gpQaPageLooksOk($breedQuestionnaireMalQueryPage)
        && str_contains($breedQuestionnaireMalQueryBody, 'belgian malinois');
    $breedQuestionnaireMalQueryAlignmentSeen = false;
    if (gpQaPageLooksOk($breedQuestionnaireMalQueryPage) && preg_match('/<h2 class="h5 mb-3">Top breed ideas<\/h2>.*?<div class="fw-bold">([^<]+)<\/div>/is', $breedQuestionnaireMalQueryPage['body'], $breedQuestionnaireMalQueryMatch)) {
        $firstBreed = trim((string) ($breedQuestionnaireMalQueryMatch[1] ?? ''));
        $breedQuestionnaireMalQueryAlignmentSeen = strcasecmp($firstBreed, 'Belgian Malinois') === 0;
    }
    $breedQuestionnaireVizQueryPage = gpQaRequest($baseUrl, 'breed_questionnaire.php', 'POST', [
        'goal' => 'companion',
        'breed_query' => 'Viz',
        'size' => 'flexible',
        'energy' => 'moderate',
    ...
    $breedQuestionnaireVizQueryBody = strtolower($breedQuestionnaireVizQueryPage['body']);
    $breedQuestionnaireVizQuerySeen = gpQaPageLooksOk($breedQuestionnaireVizQueryPage)
        && str_contains($breedQuestionnaireVizQueryBody, 'vizsla');
    $breedQuestionnaireVizQueryAlignmentSeen = false;
    if (gpQaPageLooksOk($breedQuestionnaireVizQueryPage) && preg_match('/<h2 class="h5 mb-3">Top breed ideas<\/h2>.*?<div class="fw-bold">([^<]+)<\/div>/is', $breedQuestionnaireVizQueryPage['body'], $breedQuestionnaireVizQueryMatch)) {
        $firstBreed = trim((string) ($breedQuestionnaireVizQueryMatch[1] ?? ''));
        $breedQuestionnaireVizQueryAlignmentSeen = strcasecmp($firstBreed, 'Vizsla') === 0;
    }
    $breedQuestionnaireHeelerQueryPage = gpQaRequest($baseUrl, 'breed_questionnaire.php', 'POST', [
        'goal' => 'companion',
        'breed_query' => 'Heeler',
        'size' => 'flexible',
        'energy' => 'moderate',
    ...
    $breedQuestionnaireHeelerQueryBody = strtolower($breedQuestionnaireHeelerQueryPage['body']);
    $breedQuestionnaireHeelerQuerySeen = gpQaPageLooksOk($breedQuestionnaireHeelerQueryPage)
        && str_contains($breedQuestionnaireHeelerQueryBody, 'australian cattle dog');
    $breedQuestionnaireHeelerQueryAlignmentSeen = false;
    if (gpQaPageLooksOk($breedQuestionnaireHeelerQueryPage) && preg_match('/<h2 class="h5 mb-3">Top breed ideas<\/h2>.*?<div class="fw-bold">([^<]+)<\/div>/is', $breedQuestionnaireHeelerQueryPage['body'], $breedQuestionnaireHeelerQueryMatch)) {
        $firstBreed = trim((string) ($breedQuestionnaireHeelerQueryMatch[1] ?? ''));
        $breedQuestionnaireHeelerQueryAlignmentSeen = strcasecmp($firstBreed, 'Australian Cattle Dog') === 0;
    }
    $breedQuestionnaireGreatPyrQueryPage = gpQaRequest($baseUrl, 'breed_questionnaire.php', 'POST', [
        'goal' => 'companion',
        'breed_query' => 'Great Pyr',
        'size' => 'flexible',
        'energy' => 'moderate',
        'grooming' => 'moderate',
        'public' => 'some',
    ...
    $breedQuestionnaireGreatPyrQueryBody = strtolower($breedQuestionnaireGreatPyrQueryPage['body']);
    $breedQuestionnaireGreatPyrQuerySeen = gpQaPageLooksOk($breedQuestionnaireGreatPyrQueryPage)
        && str_contains($breedQuestionnaireGreatPyrQueryBody, 'great pyrenees');
    $breedQuestionnaireGreatPyrQueryAlignmentSeen = false;
    if (gpQaPageLooksOk($breedQuestionnaireGreatPyrQueryPage) && preg_match('/<h2 class="h5 mb-3">Top breed ideas<\/h2>.*?<div class="fw-bold">([^<]+)<\/div>/is', $breedQuestionnaireGreatPyrQueryPage['body'], $breedQuestionnaireGreatPyrQueryMatch)) {
        $firstBreed = trim((string) ($breedQuestionnaireGreatPyrQueryMatch[1] ?? ''));
        $breedQuestionnaireGreatPyrQueryAlignmentSeen = strcasecmp($firstBreed, 'Great Pyrenees') === 0;
    }
    ...
    gpQaResult($results, 'breed_questionnaire_toy_manchester_alias', $breedQuestionnaireToyManchesterQuerySeen, 'HTTP ' . $breedQuestionnaireToyManchesterQueryPage['status'] . ($breedQuestionnaireToyManchesterQuerySeen ? ' toy manchester alias surfaced' : ' toy manchester alias missing'));
    gpQaResult($results, 'breed_questionnaire_toy_manchester_alignment', $breedQuestionnaireToyManchesterQueryAlignmentSeen, 'HTTP ' . $breedQuestionnaireToyManchesterQueryPage['status'] . ($breedQuestionnaireToyManchesterQueryAlignmentSeen ? ' toy manchester alias aligned to Manchester Terrier (Toy)' : ' toy manchester alias did not pin Manchester Terrier (Toy)'));
    gpQaResult($results, 'breed_questionnaire_min_pin_alias', $breedQuestionnaireMinPinQuerySeen, 'HTTP ' . $breedQuestionnaireMinPinQueryPage['status'] . ($breedQuestionnaireMinPinQuerySeen ? ' min pin alias surfaced' : ' min pin alias missing'));
    gpQaResult($results, 'breed_questionnaire_min_pin_alignment', $breedQuestionnaireMinPinQueryAlignmentSeen, 'HTTP ' . $breedQuestionnaireMinPinQueryPage['status'] . ($breedQuestionnaireMinPinQueryAlignmentSeen ? ' min pin alias aligned to Miniature Pinscher' : ' min pin alias did not pin Miniature Pinscher'));
    gpQaResult($results, 'breed_questionnaire_russian_toy_alias', $breedQuestionnaireRussianToyQuerySeen, 'HTTP ' . $breedQuestionnaireRussianToyQueryPage['status'] . ($breedQuestionnaireRussianToyQuerySeen ? ' russian toy alias surfaced' : ' russian toy alias missing'));
    gpQaResult($results, 'breed_questionnaire_russian_toy_alignment', $breedQuestionnaireRussianToyQueryAlignmentSeen, 'HTTP ' . $breedQuestionnaireRussianToyQueryPage['status'] . ($breedQuestionnaireRussianToyQueryAlignmentSeen ? ' russian toy alias aligned to Russian Toy' : ' russian toy alias did not pin Russian Toy'));
    gpQaResult($results, 'breed_questionnaire_silky_alias', $breedQuestionnaireSilkyQuerySeen, 'HTTP ' . $breedQuestionnaireSilkyQueryPage['status'] . ($breedQuestionnaireSilkyQuerySeen ? ' silky alias surfaced' : ' silky alias missing'));
    gpQaResult($results, 'breed_questionnaire_silky_alignment', $breedQuestionnaireSilkyQueryAlignmentSeen, 'HTTP ' . $breedQuestionnaireSilkyQueryPage['status'] . ($breedQuestionnaireSilkyQueryAlignmentSeen ? ' silky alias aligned to Silky Terrier' : ' silky alias did not pin Silky Terrier'));
    gpQaResult($results, 'breed_questionnaire_american_cocker_alias', $breedQuestionnaireAmericanCockerQuerySeen, 'HTTP ' . $breedQuestionnaireAmericanCockerQueryPage['status'] . ($breedQuestionnaireAmericanCockerQuerySeen ? ' american cocker alias surfaced' : ' american cocker alias missing'));
    gpQaResult($results, 'breed_questionnaire_american_cocker_alignment', $breedQuestionnaireAmericanCockerQueryAlignmentSeen, 'HTTP ' . $breedQuestionnaireAmericanCockerQueryPage['status'] . ($breedQuestionnaireAmericanCockerQueryAlignmentSeen ? ' american cocker alias aligned to American Cocker Spaniel' : ' american cocker alias did not pin American Cocker Spaniel'));
    gpQaResult($results, 'breed_questionnaire_doxie_alias', $breedQuestionnaireDoxieQuerySeen, 'HTTP ' . $breedQuestionnaireDoxieQueryPage['status'] . ($breedQuestionnaireDoxieQuerySeen ? ' doxie alias surfaced' : ' doxie alias missing'));
    gpQaResult($results, 'breed_questionnaire_doxie_alignment', $breedQuestionnaireDoxieQueryAlignmentSeen, 'HTTP ' . $breedQuestionnaireDoxieQueryPage['status'] . ($breedQuestionnaireDoxieQueryAlignmentSeen ? ' doxie alias aligned to Dachshund' : ' doxie alias did not pin Dachshund'));
    gpQaResult($results, 'breed_questionnaire_pembroke_corgi_alias', $breedQuestionnairePembrokeCorgiQuerySeen, 'HTTP ' . $breedQuestionnairePembrokeCorgiQueryPage['status'] . ($breedQuestionnairePembrokeCorgiQuerySeen ? ' pembroke corgi alias surfaced' : ' pembroke corgi alias missing'));
    ...
    gpQaResult($results, 'breed_questionnaire_blue_heeler_alias', $breedQuestionnaireBlueHeelerQuerySeen, 'HTTP ' . $breedQuestionnaireBlueHeelerQueryPage['status'] . ($breedQuestionnaireBlueHeelerQuerySeen ? ' blue heeler alias surfaced' : ' blue heeler alias missing'));
    ...
    gpQaResult($results, 'breed_questionnaire_lab_alias', $breedQuestionnaireLabQuerySeen, 'HTTP ' . $breedQuestionnaireLabQueryPage['status'] . ($breedQuestionnaireLabQuerySeen ? ' lab alias surfaced' : ' lab alias missing'));
    gpQaResult($results, 'breed_questionnaire_lab_alignment', $breedQuestionnaireLabQueryAlignmentSeen, 'HTTP ' . $breedQuestionnaireLabQueryPage['status'] . ($breedQuestionnaireLabQueryAlignmentSeen ? ' lab alias aligned to Labrador Retriever' : ' lab alias did not pin Labrador Retriever'));
    gpQaResult($results, 'breed_questionnaire_gsp_alias', $breedQuestionnaireGspQuerySeen, 'HTTP ' . $breedQuestionnaireGspQueryPage['status'] . ($breedQuestionnaireGspQuerySeen ? ' gsp alias surfaced' : ' gsp alias missing'));
    gpQaResult($results, 'breed_questionnaire_gsp_alignment', $breedQuestionnaireGspQueryAlignmentSeen, 'HTTP ' . $breedQuestionnaireGspQueryPage['status'] . ($breedQuestionnaireGspQueryAlignmentSeen ? ' gsp alias aligned to German Shorthaired Pointer' : ' gsp alias did not pin German Shorthaired Pointer'));
    gpQaResult($results, 'breed_questionnaire_mal_alias', $breedQuestionnaireMalQuerySeen, 'HTTP ' . $breedQuestionnaireMalQueryPage['status'] . ($breedQuestionnaireMalQuerySeen ? ' mal alias surfaced' : ' mal alias missing'));
    gpQaResult($results, 'breed_questionnaire_mal_alignment', $breedQuestionnaireMalQueryAlignmentSeen, 'HTTP ' . $breedQuestionnaireMalQueryPage['status'] . ($breedQuestionnaireMalQueryAlignmentSeen ? ' mal alias aligned to Belgian Malinois' : ' mal alias did not pin Belgian Malinois'));
    gpQaResult($results, 'breed_questionnaire_viz_alias', $breedQuestionnaireVizQuerySeen, 'HTTP ' . $breedQuestionnaireVizQueryPage['status'] . ($breedQuestionnaireVizQuerySeen ? ' viz alias surfaced' : ' viz alias missing'));
    gpQaResult($results, 'breed_questionnaire_viz_alignment', $breedQuestionnaireVizQueryAlignmentSeen, 'HTTP ' . $breedQuestionnaireVizQueryPage['status'] . ($breedQuestionnaireVizQueryAlignmentSeen ? ' viz alias aligned to Vizsla' : ' viz alias did not pin Vizsla'));
    gpQaResult($results, 'breed_questionnaire_heeler_alias', $breedQuestionnaireHeelerQuerySeen, 'HTTP ' . $breedQuestionnaireHeelerQueryPage['status'] . ($breedQuestionnaireHeelerQuerySeen ? ' heeler alias surfaced' : ' heeler alias missing'));
    gpQaResult($results, 'breed_questionnaire_heeler_alignment', $breedQuestionnaireHeelerQueryAlignmentSeen, 'HTTP ' . $breedQuestionnaireHeelerQueryPage['status'] . ($breedQuestionnaireHeelerQueryAlignmentSeen ? ' heeler alias aligned to Australian Cattle Dog' : ' heeler alias did not pin Australian Cattle Dog'));
    gpQaResult($results, 'breed_questionnaire_great_pyr_alias', $breedQuestionnaireGreatPyrQuerySeen, 'HTTP ' . $breedQuestionnaireGreatPyrQueryPage['status'] . ($breedQuestionnaireGreatPyrQuerySeen ? ' great pyr alias surfaced' : ' great pyr alias missing'));
    gpQaResult($results, 'breed_questionnaire_great_pyr_alignment', $breedQuestionnaireGreatPyrQueryAlignmentSeen, 'HTTP ' . $breedQuestionnaireGreatPyrQueryPage['status'] . ($breedQuestionnaireGreatPyrQueryAlignmentSeen ? ' great pyr alias aligned to Great Pyrenees' : ' great pyr alias did not pin Great Pyrenees'));
