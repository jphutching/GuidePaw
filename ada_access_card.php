...
<head>
    ...
    <style>
        :root { --gp-bg:#07111f; --gp-card:#111827; --gp-card2:#0f172a; --gp-text:#f8fafc; --gp-muted:#cbd5e1; --gp-border:rgba(255,255,255,.14); --gp-accent:#38bdf8; --gp-good:#22c55e; }
        body { background: linear-gradient(180deg, #020617, var(--gp-bg)); color: var(--gp-text); min-height: 100vh; }
        .access-shell { max-width: 980px; margin: 0 auto; padding: 1rem 1rem calc(7rem + env(safe-area-inset-bottom)); }
        .access-card { background: rgba(15,23,42,.94); border: 1px solid var(--gp-border); border-radius: 24px; padding: 1.1rem; box-shadow: 0 16px 34px rgba(0,0,0,.28); }
        .access-hero { background: radial-gradient(circle at top right, rgba(56,189,248,.18), transparent 34%), radial-gradient(circle at top left, rgba(34,197,94,.16), transparent 30%), var(--gp-card2); }
        .kicker { color: var(--gp-muted); font-weight: 700; letter-spacing: .04em; text-transform: uppercase; font-size: .78rem; }
        h1 { font-size: clamp(2.1rem, 7vw, 3.3rem); font-weight: 850; letter-spacing: -.04em; line-height: 1; }
        .muted { color: var(--gp-muted); }
        .grid { display: grid; gap: 1rem; margin-top: 1rem; }
        @media (min-width: 760px) { .grid.two { grid-template-columns: 1fr 1fr; } .utility { grid-template-columns: repeat(3, 1fr); } }
        .script { font-size: clamp(1.35rem, 4.8vw, 2rem); line-height: 1.35; font-weight: 800; }
        .qa { font-size: clamp(1.08rem, 3.8vw, 1.35rem); line-height: 1.55; font-weight: 700; }
        li + li { margin-top: .45rem; }
        .btn { border-radius: 16px; font-weight: 750; padding: .8rem 1rem; }
        .btn-outline-light { color: var(--gp-text); border-color: rgba(255,255,255,.22); }
        .pill { display:inline-flex; border:1px solid var(--gp-border); border-radius:999px; padding:.4rem .75rem; margin:.2rem .25rem .2rem 0; color:var(--gp-muted); }
        select.form-select { background:#0b1220; color:#fff; border-color: var(--gp-border); border-radius: 16px; padding:.8rem; }
        select.form-select option { color:#111827; }
        .law-title { font-size: clamp(1.35rem, 4.5vw, 2rem); font-weight: 850; line-height: 1.12; }
...
<div class="access-shell">
    ...
    <section class="grid two">
        <div class="access-card">
            <div class="kicker">Only two questions</div>
            <ol class="qa mb-0">
    ...
    <section class="grid two hide-lockscreen">
        <div class="access-card">
            <div class="d-flex justify-content-between gap-2 flex-wrap align-items-start">
                <div>
                    <div class="kicker">Federal ADA baseline</div>
                    <div class="law-title"><?= e($federalLaw['title']) ?></div>
                </div>
                <span class="status">Reviewed <?= e($federalLaw['last_reviewed']) ?></span>
            </div>
            <p class="muted mt-3"><?= e($federalLaw['summary']) ?></p>
            <ul>
                <?php foreach ($federalLaw['bullets'] as $bullet): ?>
                    <li><?= e($bullet) ?></li>
                <?php endforeach; ?>
            </ul>
            <p class="mb-0 hide-lock"><a href="<?= e($federalLaw['source_url']) ?>" target="_blank" rel="noopener"><?= e($federalLaw['source_label']) ?></a></p>
        </div>

        <div class="access-card">
            <div class="d-flex justify-content-between gap-2 flex-wrap align-items-start">
                <div>
                    <div class="kicker">State law notes</div>
                    <div class="law-title"><?= e($stateLaw['state_name']) ?></div>
                </div>
                <span class="status"><?= e($stateLaw['status'] === 'reviewed' ? 'Reviewed ' . $stateLaw['last_reviewed'] : 'Pending review') ?></span>
            </div>

            <div class="hide-lock hide-print mt-3">
                <label class="kicker" for="stateSelect">Choose state manually</label>
                <select class="form-select" id="stateSelect">
                    <?php foreach ($stateNames as $code => $name): ?>
                        <option value="<?= e($code) ?>" <?= $code === $stateCode ? 'selected' : '' ?>><?= e($name) ?></option>
                    <?php endforeach; ?>
                </select>
                <button type="button" class="btn btn-outline-light w-100 mt-2" id="gpsBtn">Use GPS to detect state</button>
                <div class="muted small mt-2" id="gpsStatus"></div>
            </div>

            <p class="muted mt-3"><?= e($stateLaw['summary']) ?></p>
            <ul>
                <?php foreach ($stateLaw['bullets'] as $bullet): ?>
                    <li><?= e($bullet) ?></li>
                <?php endforeach; ?>
            </ul>
            <div class="note mt-3"><strong>Training:</strong> <?= e($stateLaw['training_note']) ?></div>
            <div class="note mt-3"><strong>Housing:</strong> <?= e($stateLaw['housing_note']) ?></div>
            <p class="mb-0 mt-3 hide-lock">
                <?php if (!empty($stateLaw['source_url'])): ?>
                    <a href="<?= e($stateLaw['source_url']) ?>" target="_blank" rel="noopener"><?= e($stateLaw['source_label']) ?></a>
                <?php else: ?>
                    <span class="muted"><?= e($stateLaw['source_label']) ?></span>
                <?php endif; ?>
            </p>
        </div>
    </section>
    ...
    <section class="grid hide-lockscreen">
        ...
        <div class="access-card">
            ...
            <div style="font-size:clamp(1.6rem,5vw,2.35rem);font-weight:850;">800-514-0301</div>
            <p class="muted mb-0">DOJ ADA Information Line · TTY 833-610-1264</p>
        </div>
        <div class="access-card muted small">GuidePaw provides general service-animal information and official source links. This is not legal advice. Laws may vary by city, county, housing context, workplace, school, transportation setting, and facts of the situation.</div>
    </section>

    <section class="grid utility hide-print hide-lock">
        <button class="btn btn-outline-light" type="button" onclick="window.print()">Print / Save PDF</button>
...
(function () {
    ...
    function detectState(address) {
        ...
        for (var key in profiles) if (profiles[key].state_name.toLowerCase() === name) return key;
        return '';
    }

    if (gpsBtn) gpsBtn.addEventListener('click', function () {
        if (!navigator.geolocation) { status('GPS is not available. Choose your state manually.'); return; }
        status('Requesting location permission...');
        navigator.geolocation.getCurrentPosition(function (pos) {
            status('Detecting state from GPS...');
            var url = 'https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=' + encodeURIComponent(pos.coords.latitude) + '&lon=' + encodeURIComponent(pos.coords.longitude) + '&zoom=5&addressdetails=1';
            fetch(url, {headers: {'Accept': 'application/json'}}).then(function (r) { return r.json(); }).then(function (data) {
                var code = detectState(data.address || {});
                if (code) { status('Detected ' + profiles[code].state_name + '. Loading...'); go(code); }
                else { status('Could not detect a supported state. Choose your state manually.'); }
            }).catch(function () { status('Could not reverse lookup GPS location. Choose your state manually.'); });
        }, function (err) { status(err && err.message ? err.message : 'Location permission was not granted. Choose your state manually.'); }, {enableHighAccuracy:false, timeout:12000, maximumAge:300000});
    });
})();
</script>
</body>
