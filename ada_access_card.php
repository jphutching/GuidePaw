...
<section class="grid hide-lockscreen" id="official-sources">
<div class="access-card source-callout"><div class="kicker">Official source first</div><div class="law-title">Start with ADA.gov guidance</div><p class="muted mt-3 mb-2">For the most current federal public-access guidance, use the Department of Justice ADA.gov Service Animals FAQ. GuidePaw helps organize information, but official sources control.</p><p class="mb-0"><a href="https://www.ada.gov/resources/service-animals-faqs/" target="_blank" rel="noopener">Open ADA.gov Service Animals FAQ</a></p><div class="quick-links hide-print"><a href="#service-types">Service Dog vs SDIT vs ESA</a><a href="#state-law-notes">State law notes</a><a href="air_travel_rights.php">GuidePaw air travel page</a><a href="#official-air-travel">Official air travel rules</a><a href="#registration-warning">Registration warning</a></div></div>
</section>
...
<section class="grid hide-lockscreen" id="service-types">
<div class="access-card"><div class="kicker">Service Dog, SDIT, ESA</div><div class="law-title">Different labels have different protections</div><p class="muted mt-3">Public access, housing, and air travel do not use one identical rule. Use this as a quick orientation and check the official source links below.</p><div class="table-responsive"><table class="comparison-table"><thead><tr><th>Category</th><th>ADA public access</th><th>Housing</th><th>Air travel</th></tr></thead><tbody><tr><td data-label="Category"><strong>Fully trained service dog</strong><br><span class="muted">A dog trained to perform disability-related work or tasks.</span></td><td data-label="ADA public access">Generally protected under ADA public-access rules when under control and housebroken.</td><td data-label="Housing">May qualify as an assistance animal depending on the housing facts and law.</td><td data-label="Air travel">Handled under DOT/ACAA airline rules and may require DOT forms.</td></tr><tr><td data-label="Category"><strong>Service dog in training / SDIT</strong><br><span class="muted">A dog still learning tasks or public access skills.</span></td><td data-label="ADA public access">Not a service animal under the federal ADA until trained; some state or local laws may give access rights.</td><td data-label="Housing">Depends on the housing facts, documentation rules, and applicable law.</td><td data-label="Air travel">Not treated as a service animal under DOT air-travel rules.</td></tr><tr><td data-label="Category"><strong>ESA / comfort / companion animal</strong><br><span class="muted">Provides comfort or emotional support by presence alone.</span></td><td data-label="ADA public access">Not a service animal under the ADA unless trained to perform disability-related work or tasks.</td><td data-label="Housing">May be considered an assistance animal as a reasonable accommodation under fair-housing rules.</td><td data-label="Air travel">Not treated as a service animal under DOT air-travel rules.</td></tr></tbody></table></div></div>
</section>
...
<div class="access-card" id="state-law-notes"><div class="d-flex justify-content-between gap-2 flex-wrap align-items-start"><div><div class="kicker">State law notes</div><div class="law-title"><?= e($stateLaw['state_name']) ?></div></div><span class="status"><?= e($stateLaw['status'] === 'reviewed' ? 'Reviewed ' . $stateLaw['last_reviewed'] : 'Pending review') ?></span></div><div class="hide-lock hide-print mt-3"><label class="kicker" for="stateSelect">Choose state manually</label><select class="form-select" id="stateSelect"><?php foreach ($stateNames as $code => $name): ?><option value="<?= e($code) ?>" <?= $code === $stateCode ? 'selected' : '' ?>><?= e($name) ?></option><?php endforeach; ?></select><button type="button" class="btn btn-outline-light w-100 mt-2" id="gpsBtn">Use GPS to detect state</button><div class="muted small mt-2" id="gpsStatus"></div><div class="muted small mt-2">When location access is already granted, GuidePaw will try to auto-detect your state on load.</div></div><p class="muted mt-3"><?= e($stateLaw['summary']) ?></p><ul><?php foreach ($stateLaw['bullets'] as $bullet): ?><li><?= e($bullet) ?></li><?php endforeach; ?></ul><div class="note mt-3"><strong>Training:</strong> <?= e($stateLaw['training_note']) ?></div><div class="note mt-3"><strong>Housing:</strong> <?= e($stateLaw['housing_note']) ?></div><p class="mb-0 mt-3 hide-lock"><?php if (!empty($stateLaw['source_url'])): ?><a href="<?= e($stateLaw['source_url']) ?>" target="_blank" rel="noopener"><?= e($stateLaw['source_label']) ?></a><?php else: ?><span class="muted"><?= e($stateLaw['source_label']) ?></span><?php endif; ?></p></div>
</section>
<section class="grid hide-lockscreen" id="registration-warning"><div class="access-card warning-card"><div class="kicker">Registration and certification warning</div><div class="law-title">Online registration does not create ADA rights</div><p class="muted mt-3 mb-0">The ADA does not require a service dog registry, certificate, ID card, vest, or online registration for public access. Documents sold online as “service dog registration” do not create ADA rights and are not recognized by the Department of Justice as proof that a dog is a service animal. Local dog licensing and vaccination rules may still apply to service dogs the same way they apply to other dogs.</p></div></section>
<section class="grid two hide-lockscreen" id="official-air-travel"><div class="access-card"><div class="kicker">Official air travel rules</div><div class="law-title">DOT / airline service animal guidance</div><p class="muted mt-3">Airline travel follows DOT Air Carrier Access Act rules, not the ADA public-access rule. Airlines may require DOT service animal forms and may deny transport in specific safety or behavior situations.</p><p class="mb-0"><a href="https://www.transportation.gov/resources/individuals/aviation-consumer-protection/service-animals" target="_blank" rel="noopener">Open DOT service animal air travel page</a></p></div><div class="access-card"><div class="kicker">Housing source</div><div class="law-title">HUD assistance animal guidance</div><p class="muted mt-3">Housing uses a different framework from stores, restaurants, and other public-access locations. HUD guidance covers assistance animals, reasonable accommodations, and emotional support animals in housing contexts.</p><p class="mb-0"><a href="https://www.hud.gov/helping-americans/assistance-animals" target="_blank" rel="noopener">Open HUD assistance animals page</a></p></div></section>
